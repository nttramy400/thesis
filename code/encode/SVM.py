...
##        
    ...
    kf = KFold(n_splits=5)
    for k_KNN_feature_selection in range(50,100): #k neightbor seq in knn graph
...
#        plt.plot(kFeatures, sumarizeAccuracy, "g-o", label="Laplacian score_KNN")
        plt.plot(kFeatures, sumarizeAccuracy2,"r-o", label="Laplacian score_LabelPropagation")
        plt.ylabel('accuracy')
        plt.xlabel('number of features')
        plt.legend()
#        plt.show()
        plt.savefig("..//outputfile//"+version+ "//"+testVer+"//knnGraph_"+str(k_KNN_feature_selection)+"-LabelPropagation.png")
        plt.close()
