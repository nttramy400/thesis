...
def buildWeightMat(seqRowMat, labelVec, kNumOfNeighbors):
    logging.info("Build within-class graph")
...
def buildBetweenGraph(seqRowMat, labelVec):
    logging.info("Build between-class graph")
    numOfSeq = len(seqRowMat)
    numOfLabelSeq = len(labelVec)
    betweenMat = np.zeros((numOfSeq, numOfSeq))
    ...
    for index1 in range(numOfLabelSeq):
        for index2 in range(index1, numOfLabelSeq):
            if labelVec[index1] != labelVec[index2]: 
                betweenMat[index1][index2] = betweenMat[index2][index1] = 1
    return betweenMat
...
def computeLaplacianScore(vectorFeature,laplaMat, laplaBetweenMat):
#    normFeatureVec = normalizeFeature2(vectorFeature,diagleMat)
    normFeatureVec = vectorFeature
    return np.asscalar((np.transpose(normFeatureVec).dot(laplaMat).dot(normFeatureVec))
                / (np.transpose(normFeatureVec).dot(laplaBetweenMat).dot(normFeatureVec))) #np.asscalar convert array 1 element to scarlar
...
def calculateAllLaplacianScore(k_knn_graph):
    ...
    betweenGraph = buildBetweenGraph(seqRowMat, labelVec)
#        print datetime.now() - startTime
    del seqRowMat
    diagleMat = buildDiagleMat(weightMat)
    laplaMat = buildLaplacianMat(weightMat, diagleMat)  
    
    diagleBetweenMat = buildDiagleMat(betweenGraph)
    laplaBetweenMat = buildLaplacianMat(betweenGraph, diagleBetweenMat) 
    
    
    featureRowMat = h5filereader.readFeatureRowMatFromFile()
    listLaplacianScore = [computeLaplacianScore(np.transpose([featureVec]),laplaMat, laplaBetweenMat) for featureVec in featureRowMat]
    #save to file
