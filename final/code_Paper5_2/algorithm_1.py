...
def computeLaplacianScore(vectorFeature,laplaMat, diagleBetweenMat):
#    normFeatureVec = normalizeFeature2(vectorFeature,diagleMat)
    ...
    return np.asscalar((np.transpose(normFeatureVec).dot(laplaMat).dot(normFeatureVec))
                / (np.transpose(normFeatureVec).dot(diagleBetweenMat).dot(normFeatureVec))) #np.asscalar convert array 1 element to scarlar
...
#        print datetime.now() - startTime
    ...
    listLaplacianScore = [computeLaplacianScore(np.transpose([featureVec]),laplaMat, diagleBetweenMat) for featureVec in featureRowMat]
    #save to file
